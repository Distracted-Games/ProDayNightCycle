...
- **Lifecycle Ready**: Integrates seamlessly with my [`ModuleLoader`](https://github.com/Distracted-Games/ModuleLoader) framework (`Setup` and `Start`).
...
### With ModuleLoader
The module is designed to work out-of-the-box. If you're using the [`ModuleLoader`](https://github.com/Distracted-Games/ModuleLoader) framework, the `ModuleLoader` will automatically call its `Start()` method, beginning the cycle with the default configuration.

### Standalone
If you're not using `ModuleLoader`, you can start the cycle manually (make sure to remove the refefences to `ModuleLoader`):
...
Debug = true
```
in the logger configuration inside the module. This will print detailed messages to the output about time progression and event triggers.
